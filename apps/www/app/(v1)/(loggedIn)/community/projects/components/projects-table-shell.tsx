  ...
  const columns = useMemo<ColumnDef<IProject, unknown>[]>(
    () => [
      ...
      table.simpleColumn("Ref/Date", (data) => ({
        story: [table.primaryText(data.refNo), table.secondary(data.createdAt)],
      })),
...

      {
        ...
        cell: ({ row }) => (
          <RowActionCell>
            <EditRowAction
              onClick={() => {
                modal.openModal(<ProjectModal data={row.original} />);
              }}
            />
            <DeleteRowAction row={row.original} action={deleteProjectAction} />
          </RowActionCell>
        ),
