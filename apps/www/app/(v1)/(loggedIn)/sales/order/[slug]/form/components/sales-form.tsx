...
export default function SalesForm({ data, newTitle, slug }: Props) {
    ...
    debounce(() => {
      form.handleSubmit((d) => {
        if (d.customerId) {
          ...
          toast.error("Autosave paused, requires customer information.");
        }
      })();
  ...
  useEffect(() => {
    ...
    const { _items, footer } = salesUtils.initInvoiceItems(resp?.form?.items);
      ...
      startTransition(async () => {
        ...
        if (response.orderId) {
          ...
          if (and == "close") router.push(`/sales/${type}s`);
            ...
            if (and == "new") router.push(`/sales-book/create-${type}`);
            else {
          ...
          <section id="header" className="flex items-center justify-between">
            ...
            <div className="flex-1 px-4">
              <Button asChild variant={"destructive"} size="sm">
                <Link href={`/sales/edit/${data.form.type}/${data.form.slug}`}>
                  {/* <Icons.Rocket /> */}
                  Switch to V2
                </Link>
              </Button>
