      ...
      <Card className="max-sm:border-none">
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            <div className="inline-flex items-center space-x-2">
              <span>{estimate ? "Quote " : "Order "} Information</span>
              {/* <OrderPriorityFlagCell order={order as any} /> */}
              <StatusBadge status={order?.prodStatus || "no status"} />
            </div>

            {isProd ? <></> : <OrderOverviewActions estimate={estimate} />}
          </CardTitle>
        </CardHeader>
        <CardContent className="divide-y space-y-4 flex flex-col">
          <div className="grid grid-cols-2 gap-4 text-sm sm:grid-cols-3">
            ...
            <Info label={order.type == "quote" ? "Quote Date" : "Order Date"}>
            ...
            {order.type == "quote" ? (
              ...
              <>
                <Info label="Payment Terms">
                  ...
                  <p>{formatDate(order.paymentDueDate as any)}</p>
                </Info>
              </>
            )}
...
function AddressInfo({
  ...
  return (
    <Info label={label}>
      <p>{address?.name}</p>
      <p>
        {address?.phoneNo} {address?.phoneNo2 ? `(${address?.phoneNo2})` : ""}
      </p>
