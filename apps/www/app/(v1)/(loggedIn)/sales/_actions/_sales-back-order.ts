...
export async function _createSalesBackOrder(
  ...
  let newOrderItems = items
    ?.map((_item) => {
      ...
      let hasTax = item.meta.tax == salesFormUtils.taxxable(item.meta.tax);
      ...
      if (backOrder?.backQty) {
        ...
        if (hasTax && taxPercent) {
          ...
          itemUpdate.tax = +toFixed(itemUpdate.total * (taxPercent / 100));
  ...
  if (amountDue >= newGrandTotal) {
    ...
    payments?.map((p) => {
      ...
      if (p.amount >= rem) {
        let _p = (paymentUpdate.updates[p.id] = +toFixed(p.amount - rem));
