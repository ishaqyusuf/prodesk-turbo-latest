    ...
    startTransition(async () => {
      ...
      try {
        ...
        if (type == "tasks") {
          ...
          const b = await _updateBuilderMetaAction(data.meta, data.id);
          ...
          await toastArrayAction({
            ...
            serverAction: async (units) =>
              await _syncBuilderTasks(data, deleteIds, newTaskIds, units),
            loading(item) {
        ...
        if (type == "installations") await saveBuilderInstallations(data);
...
function TasksForm({}) {
  ...
  return (
    <div className="col-span-2 grid gap-2">
      ...
      {fields?.map((f, i) => (
        <div className="grid grid-cols-11 gap-2 items-center group" key={i}>
