  ...
  const columns = useMemo<ColumnDef<ICommunityTemplate, unknown>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <>
            <PrimaryCellContent>{row.original._count.homes}</PrimaryCellContent>
          </>
...

      {
        ...
        cell: ({ row }) => (
          <RowActionCell>
            <EditRowAction
              onClick={(e) => openModal("communityTemplate", row.original)}
            />
            <DeleteRowAction
              noRefresh
              row={row.original}
                  ...
                  async function __importCost() {
                    ...
                    async function updateCosts(index) {
                      ...
                      if (_cost)
                        toast.promise(
                          async () => {
                            await _synchronizeModelCost(_cost, row.original.id);
                            await timeout(1000);
                            return true;
                          },
                          {
                    ...
                    console.log(row.original.project.builder.meta.tasks);
                    ...
                    if (!_) toast.error("No Import found");
                      ...
                      toast.success("Cost Import Successfully");
                      await updateCosts(0);
                    }
                  }
                  if (row.original.pivot?._count.modelCosts) {
                    toast(
                      "Model contains costs, this action will override existing costs. Proceed?",
                      {
                        action: {
                          label: "Yes",
                          onClick: async () => await __importCost(),
                        },
                      },
                    );
