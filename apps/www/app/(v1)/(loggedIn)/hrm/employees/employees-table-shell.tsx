  ...
  const columns = useMemo<ColumnDef<IUser, unknown>[]>(
    () => [
      ...
      table.simpleColumn("Name", (data) => ({
        story: [table.primaryText(data.name), table.secondary(data.username)],
      })),
          ...
          <DropdownMenu key={1}>
            <DropdownMenuTrigger asChild>
              ...
              >
                <span className="whitespace-nowrap">
                  {data.employeeProfile?.name || "Select Profile"}
                </span>
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-[185px]">
              {profiles.data?.map((profile) => (
                <DropdownMenuItem
                  onClick={() => setEmployeeProfile(data.id, profile)}
                  key={profile.id}
                >
                  {profile.name}
                </DropdownMenuItem>
      ...
      ..._FilterColumn("_q", "_roleId"),
      {
        ...
        cell: ({ row }) => (
          <RowActionCell>
            <DeleteRowAction row={row.original} action={_deleteEmployee} />
            <RowActionMoreMenu>
              <RowActionMenuItem
                onClick={() => {
                  modal.openModal(<EmployeeForm defaultData={row.original} />);
                }}
                Icon={Icons.edit}
              >
                Edit
