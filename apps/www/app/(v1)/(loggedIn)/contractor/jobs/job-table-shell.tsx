      ...
      CheckColumn({ selectedRowIds, setSelectedRowIds, data }),
      {
        ...
        cell: ({ row }) => (
          <Cell
            ...
            onClick={() => {
              modal.openSheet(
                <JobOverviewSheet job={row.original} admin={payment} />,
              );
            }}
          >
            <PrimaryCellContent>
              {row.original.title || "-"} <JobType job={row.original} />
            </PrimaryCellContent>
            <SecondaryCellContent>
        ...
        [
          {
            ...
            cell: ({ row }) => (
              <RowActionCell>
                {row.original.status?.toLowerCase() == "assigned" && (
                  <>
                    <Button
                      onClick={() => {
                        openModal("submitJob", {
                ...
                <DeleteRowAction
                  ...
                  disabled={
                    row.original.paymentId > 0 || row.original.homeTasks?.length
                  }
                />
                <RowActionMoreMenu>
                  <RowActionMenuItem
                    ...
                    onClick={() => {
                      modal.openModal(<SubmitJobModal job={row.original} />);
                    }}
                    Icon={Icons.edit}
                  >
                    Edit
