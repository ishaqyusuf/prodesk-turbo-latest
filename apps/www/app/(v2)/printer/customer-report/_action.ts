...
export async function generateCustomerPrintReport(
    ...
    .map((order) => {
      ...
      if (orderDue > 0) {
        ...
        if (order.paymentDueDate) {
          ...
          else if (days > 60 && days <= 90) reportFooter.past3 += orderDue;
          else if (days > 30 && days <= 60) reportFooter.past2 += orderDue;
          else if (days >= 1 && days <= 30) reportFooter.past1 += orderDue;
          console.log({
            days,
            orderDue,
          });
  ...
  return {
    customer: [customer.businessName, customer.name, customer.address].filter(
      Boolean,
    ),
    reportTable,
    reportFooter,
