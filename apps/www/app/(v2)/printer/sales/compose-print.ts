...
function getDoorsTable(
  ...
  const dt = {
    ...
    doors: data.order.items
      .filter(
        (item) => item.housePackageTool || item?.meta?.doorType == "Services",
      )
      .filter(
...
function printFooter(data: PrintData, notPrintable) {
  ...
  return {
    lines: [
      styled("Subtotal", formatCurrency.format(data.order.subTotal || 0), {
        font: "bold",
      }),
      ...taxLines,
      styled("Labor", formatCurrency.format(data.order.meta?.labor_cost || 0), {
        font: "bold",
      }),
      data.order.meta?.ccc
        ? styled("C.C.C", formatCurrency.format(data.order.meta.ccc || 0), {
            font: "bold",
          })
        : null,
      data.order.meta.deliveryCost > 0
        ...
        : null,
      totalPaid > 0
        ? styled("Total Paid", `(${formatCurrency.format(totalPaid || 0)})`, {
            font: "bold",
          })
        : null,
      styled("Total Due", formatCurrency.format(data.order.amountDue || 0), {
        font: "bold",
        size: "base",
      }),
