    ...
    <Modal.Content>
      ...
      <div className="overflow-auto h-[90vh] -px-8">
        <div className="flex gap-4 py-4">
          ...
          />
          <Info label="Pending" value={<Money value={ctx.data?.amountDue} />} />
          <Info label="Payment Term" value={ctx.data?.paymentTerm || "-"} />
          <Info
            label="Invoice Due Date"
            value={
              ctx.data?.paymentDueDate ? (
                <TableCol.Date>{ctx.data?.paymentDueDate}</TableCol.Date>
              ) : (
                "-"
              )
            }
            ...
            <TableBody>
              {ctx.data?.payments?.map((payment) => (
                <TableRow key={payment.id}>
                  <TableCell>
                    <TableCol.Date>{payment.createdAt}</TableCol.Date>
                  </TableCell>
                  <TableCell>
                    <TableCol.Money value={payment.amount} />
                    <TableCol.Secondary>
        ...
        <div className="grid gap-2">
          <Button
            ...
            size="sm"
            onClick={() => {
              modal.openModal(<SquarePaymentModal id={ctx.data.id} />);
            }}
          >
            Square Checkout
          </Button>
          ...
          <Form {...form}>
            <div className="grid grid-cols-2 gap-4 border rounded p-2 mb-10">
              ...
              <div className="col-span-2 space-x-4 flex justify-end">
                ...
                <Btn isLoading={loading} size={"sm"} onClick={pay}>
                  Pay
                </Btn>
              </div>
            </div>
