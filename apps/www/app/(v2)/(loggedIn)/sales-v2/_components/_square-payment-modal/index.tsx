...
export default function SquarePaymentModal({ id }: { id: number }) {
  ...
  async function createPayment() {
    ...
    // return;
    try {
      const data = order.dealerMode ? defaultFormData() : form.getValues();
      if (data.type == "terminal") {
        if (!data.deviceId) throw new Error("Select a terminal");

        if (env.NEXT_PUBLIC_NODE_ENV == "production") {
          const devices = await getSquareDevices();
          const selectedDevice = devices.find((d) => d.value == data.deviceId);
      ...
      if (data.type == "terminal") {
        if (resp.salesCheckoutId) {
          ...
          form.setValue("modalSubtitle", "Swipe your card to finalize payment");
          setTab("processingPayment");
        } else {
        }
      } else {
      ...
      <Modal.Content>
        ...
        <Tabs value={tab} onValueChange={setTab as any}>
          ...
          <TabsContent value="paymentLinkForm">
            ...
            <div className="mt-4">
              <Modal.Footer submitText="Proceed" onSubmit={createPayment} />
            </div>
