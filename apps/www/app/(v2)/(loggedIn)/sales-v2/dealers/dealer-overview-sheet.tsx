...
export default function DealerOverviewSheet({ dealer }: Props) {
  ...
  async function _action(status: DealerStatus) {
    ...
    if (status == "Approved") {
      await updateDealerProfileAction(dealer.id, +form.getValues("profileId"));
    ...
    <Modal.Content>
      ...
      <div className="grid grid-cols-1 gap-4 mt-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Info label="Status" value={<StatusBadge status={dealer.status} />} />
          <Info
            label="Application Date"
            value={<TableCol.Date>{dealer.createdAt}</TableCol.Date>}
          />
        </div>
      </div>
