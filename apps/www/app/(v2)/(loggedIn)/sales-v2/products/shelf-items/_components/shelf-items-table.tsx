    ...
    (ctx) => [
      ctx.Column("Product", ({ item }) => (
        <TableCol>
          ...
          <TableCol.Secondary>{item.category?.name}</TableCol.Secondary>
        </TableCol>
      )),
      ...
      ctx.ActionColumn(({ item }) => (
        <>
          <TableCol.Btn
            icon="edit"
            onClick={(e) => {
              modal?.openModal(<ShelfItemFormModal data={item as any} />);
            }}
          />
          <TableCol.DeleteRow action={deleteDykeShelfItem} data={item} />
        </>
      )),
