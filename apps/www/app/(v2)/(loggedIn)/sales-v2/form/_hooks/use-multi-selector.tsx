...
export function useMultiSelector(rowIndex, get) {
  ...
  const multi = {
    async initServices() {
      ...
      form.setValue(`itemArray.${rowIndex}.multiComponent.multiDyke`, true);
    ...
    watchMultiComponent() {
      return form.watch(`itemArray.${rowIndex}.multiComponent.components`);
    },
    ...
    validateMultiSelect(products: IStepProducts, stepFormTitle) {
      ...
      // console.log(items);
      const checkedItems = Object.values(items || {}).filter((b) => b?.checked);
      const isMoulding = stepFormTitle == "Moulding";
      const pkdId = get.packageToolId(isMoulding ? "molding" : "dykeDoor");
      // console.log(items);
      ...
      //     rowIndex
      // );
      form.setValue(`itemArray.${rowIndex}.multiComponent.rowIndex`, rowIndex);
      return prod;
    },
    select(currentState, stepProd: IStepProducts[0], stepFormTitle, onSelect) {
      ...
      if (!uid)
        form.setValue(`${basePath}.uid` as any, generateRandomString(4));
      if (!isMoulding) {
