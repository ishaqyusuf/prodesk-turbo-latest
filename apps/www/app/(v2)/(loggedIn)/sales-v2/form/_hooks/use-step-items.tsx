...
export default function useStepItems(stepCtx: LegacyDykeFormStepType) {
  ...
  async function selectProduct(
    ...
    ctx.startLoadingStep(async () => {
      ...
      if (!isMultiSection) {
        const baseUnitCost = (data.basePrice = stepProd._metaData?.basePrice);
      ...
      switch (stepTitle) {
        case "Height":
          ...
          form.setValue(`itemArray.${item.rowIndex}.item.housePackageTool`, {
            height: val,
            totalDoors: 0,
            totalPrice: 0,
            doorType: _doorType,
      ...
      for (let si = stepIndex; si > -1; si--) {
        ...
        if (formSeq) {
          formSeq.map(({ id: fsId }) => {
            const existingStep = formSteps.find((f) => f.step.id == fsId);
