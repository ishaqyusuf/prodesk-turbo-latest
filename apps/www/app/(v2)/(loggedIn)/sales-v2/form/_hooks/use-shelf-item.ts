  ...
  const ctx = {
    ...
    productSelected(productId, prodIndex) {
      ...
      if (prod) {
        ...
        const uid =
          form.getValues(`${configky}.uid` as any) || generateRandomString(4);
    ...
    watchProductEstimate(index) {
      return form.watch(this.getProdFormKey(index, "unitPrice", "totalPrice"));
    },
    clearEstimates() {
    ...
    productUpdated(product: IDykeShelfProducts, prodIndex) {
      setProducts((current) => {
        const cIndex: any = current?.findIndex((p) => p.id == product.id);
        ...
        if (prodIndex != null) {
          ...
          const prodKey: any = `${configky}.productArray.${prodIndex}.item`;
          form.setValue(`${prodKey}.unitPrice` as any, product?.unitPrice);
          form.setValue(`${prodKey}.description` as any, product?.title);
          // const qty = form.getValues(`${prodKey}.qty` as any);
          // if(!qty)
          // form.setValue
          // form.setValue()
