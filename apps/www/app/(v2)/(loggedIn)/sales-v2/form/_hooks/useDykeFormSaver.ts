...
export default function useDykeFormSaver(form) {
  ...
  function save(data: DykeForm, mode: SaveMode = "default", onComplete = null) {
    ...
    startTransition(async () => {
      ...
      try {
        ...
        if (e.order.type == "order") {
          e.order.paymentDueDate = salesFormUtils._calculatePaymentTerm(
            e.order.paymentTerm,
            e.order.createdAt,
          );
        ...
        if (!errorId) await _saveDykeError(errorData.errorId, errorData);
      }
    });
  }
