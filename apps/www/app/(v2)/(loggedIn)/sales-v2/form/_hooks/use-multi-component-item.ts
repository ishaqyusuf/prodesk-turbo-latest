...
export function useMultiComponentItem(componentTitle) {
  ...
  useEffect(() => {
    ...
    Object.entries(c).map(([title, data]) => {
      if (!data) return;
      const p = componentTitle == title ? _totalPrice : data.totalPrice || 0;
  ...
  function calculateSizeEstimate(dim?, qty?, _totalLinePrice?) {
    ...
    Object.entries(itemData.multiComponent.components).map(([title, cData]) => {
      if (title == componentTitle) {
        let totalDoors = 0;
        let unitPrice = 0;
        let totalPrice = 0;
        Object.entries(cData._doorForm).map(([_size, _doorForm]) => {
...
export function useMultiComponentSizeRow(
  ...
  const { form, multiComponentComponentTitleKey, prices, item } = componentItem;
