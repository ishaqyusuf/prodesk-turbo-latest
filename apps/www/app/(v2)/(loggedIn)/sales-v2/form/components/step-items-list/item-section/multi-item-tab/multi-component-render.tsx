          ...
          {line ? (
            <div className="max-h-[300px] px-8 -mx-8 overflow-auto">
              <Table id="housePackageTable">
                <TableHeader>
                  ...
                  <TableHead>
                    {item.isType.moulding ? "Moulding" : "Description"}
                  </TableHead>
                  {item.isType.service && (
                    <>
                ...
                <TableBody>
                  {mdf.tabs?.map((tab, index) => (
                    <TableRow
                      className={cn(tab.deleted && "hidden", "border-none")}
                      key={index}
                    >
                      <TableCell>
                        {mdf.tabs
                          .filter((t, i) => i < index)
                          .filter((t) => !t.deleted).length + 1}
                      </TableCell>
                      <Render mdf={mdf} componentTitle={tab.title} />
                    </TableRow>
                  ))}
                  <TableRow>
                    <TableCell></TableCell>
            ...
            >
              <TabsList
                ...
                className={cn(line && "flex flex-col w-1/3", "h-auto")}
              >
                {mdf.tabs?.map((tab, index) => (
                  <TabsTrigger
                    value={tab.title}
