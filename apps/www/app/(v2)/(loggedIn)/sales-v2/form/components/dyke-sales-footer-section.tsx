...
type ICtx = typeof defaultValues & {
  taxForm?: {
    ...
    selectionArray?: UseFieldArrayReturn<DykeForm, "_taxForm.selection", "_id">;
    taxSelect;
    setTaxSelect;
  };
};
...
export default function DykeSalesFooterSection({}) {
  ...
  useEffect(() => {
    ...
    Object.entries(estimate.taxForm.taxByCode).map(([k, v]) => {
      ...
      form.setValue(`_taxForm.taxByCode.${k}.data.taxxable`, v.data?.taxxable);
    });
          ...
          <Table>
            <TableBody>
              <TableRow>
                ...
                <Details.Line title="Sub Total:" valueKey="subTotal" />
                <Details.Line
                  title={`Tax (${_ctx.orderTax ? _ctx.taxPercentage : 0}%):`}
                  valueKey="tax"
                />
          ...
          <TableRow key={i}>
            ...
            <TableCell className="w-full">
              <div className="flex justify-end  w-full space-x-2 items-center">
                ...
                <Button
                  variant="destructive"
                  onClick={() => footerCtx.removeTaxSelection(s.taxCode, i)}
                  className="w-8 h-8"
                  size="icon"
                >
                  <Icons.X />
      ...
      <TableRow>
        <TableHead className="bg-red-400 w-full" colSpan={2}>
          <div className="flex items-center gap-4">
            ...
            <Select value={selection} onValueChange={selectionChanged}>
