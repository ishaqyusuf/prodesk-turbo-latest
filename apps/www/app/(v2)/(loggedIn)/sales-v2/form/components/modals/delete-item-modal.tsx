  ...
  async function submit() {
    ...
    const _stepItems = await Promise.all(
      stepItems.map(async (stepItem) => {
        ...
        const stateDeps = {};
        Object.entries({ ...stepItemMeta.deleted, ...d }).map(([a, b]) => {
          if (b) {
            stateDeps[a] = true;
            delete stepItemMeta.show?.[a];
            Object.entries(stepItemMeta.show || {}).map(([k, v]) => {
              if (k?.includes(a)) delete stepItemMeta.show?.[k];
            });
          }
        });
      ...
      <Modal.Content>
        {deletables?.length ? (
          <>
            <Modal.Header
              title={"Delete"}
              subtitle={"Select component combination to delete this item for"}
            />
            <div className="">
              <Table>
                <TableBody>
          ...
          <>
            ...
            <div className="">
              <span>This component has no dependencies. Continue delete?</span>
            </div>
            <Modal.Footer
              submitText="Delete"
              cancelText="Cancel"
