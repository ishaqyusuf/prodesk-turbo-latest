    ...
    setProds(
      (products as IStepProducts)
        ...
        .map((product) => {
          ...
          let importables = [{ title: "All", count: 0, priceData: {} }];
          let heights = Array.from(
            new Set(
              Object.keys(pricings).map((k) => k.split(" ").reverse()[0]),
            ),
          );
          ...
          heights.map((h) => {
            let priceData = {};
            const _p = prices.filter((a) => a.height == h && a.price);
            _p.map((p) => {
                  ...
                  SubMenu={
                    <>
                      ...
                      <div className="flex">
                        ...
                        <div
                          className={cn("w-48 h-48", !p.doorImg && "hidden")}
                        >
                          <ProductImage
                            aspectRatio={4 / 4}
                            item={{
