    ...
    (async () => {
      ...
      Object.entries(heights || {}).map(([k, v]) => {
        ...
        _defData[k] = {
          ...
          price: salesFormUtils.salesProfileCost(form, heightPrices[s?.dimFt]),
        };
      });
      sizeForm.reset({
        sizes: _defData,
    ...
    Object.entries(sizesData || {}).map(([dim, { checked }]) => {
      ...
      if (checked && size) {
        ...
        const price = (priceTags.doorSizePriceTag[size.dimFt] =
          size.price || 0);
        const jamPath = `${baseKey}._doorForm.${size.dim}.jambSizePrice` as any;
        form.setValue(jamPath, price);
        const pDataKey = `${baseKey}._doorForm.${size.dim}.priceData` as any;
        const pData = form.getValues(pDataKey) || {};
        form.setValue(
          pDataKey,
          salesFormUtils.componentPrice.update(form, pData, size.basePrice),
        );
      }
    });
    const checked = (Object.values(sizesData).filter((s) => s.checked)?.length >
      0) as any;
    ...
    return (
      <div className="flex border justify-between p-3 py-2 group items-start">
        ...
        <DropdownMenu open={show} onOpenChange={setShow}>
          ...
          >
            <Button size="sm" variant="outline" className="h-8 w-8 p-1">
    ...
    <Modal.Content size="lg">
      ...
      <Form {...sizeForm}>
        ...
        <form
          ...
          onSubmit={(...args) => void form.handleSubmit(createNewSize)(...args)}
        >
      ...
      <Modal.Footer submitText="Proceed" onSubmit={_onSubmit}></Modal.Footer>
    </Modal.Content>
  );
}
