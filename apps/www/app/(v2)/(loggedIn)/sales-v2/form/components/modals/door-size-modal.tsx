...
export default function DoorSizeModal({
  ...
  function onSubmit() {
    ...
    const basePriceTags = form.getValues(`${basePath}.priceTags` as any) || {};
  ...
  return (
    <Form {..._form}>
      <Modal.Content size="lg">
        <Modal.Header title={"Select Door Sizes"} subtitle={productTitle} />
        <div className="">
          <Table className="table-fixed size-sm">
            <TableHeader>
              <TableRow>
                <TableHead colSpan={4}>Size</TableHead>
                {isType.hasSwing && <TableHead colSpan={3}>Swing</TableHead>}
                <TableHead colSpan={2}>
                  {isType.multiHandles ? "LH" : "Qty"}
                </TableHead>
                {isType.multiHandles && <TableHead colSpan={2}>RH</TableHead>}
              </TableRow>
            </TableHeader>
          </Table>
                  ...
                  >
                    <TableCell colSpan={4}>
                      <div className="flex justify-between">
                        <TableCol.Primary>{size.dimFt}</TableCol.Primary>
                        <TableCol.Secondary className={cn("")}>
                          {size.basePrice > 0 && (
                            <div className="flex gap-2">
                              <Badge>
                                <Money value={size.basePrice} />
                              </Badge>
                              {superAdmin && (
