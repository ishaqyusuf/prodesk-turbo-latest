...
export function SectionedItemAssignForm({ index, salesDoorIndex = -1 }: Props) {
  ...
  const modal = useAssignment(data.data.isProd ? { type: "prod" } : undefined);
  ...
  return (
    <DropdownMenu open={open} onOpenChange={onOpenChange}>
      <Button
        ...
        disabled={
          salesDoorIndex >= 0
            ? group.salesDoors[salesDoorIndex]?.report?.pendingAssignment == 0
      ...
      >
        <DropdownMenuTrigger>
          ...
          <span>
            (
            {salesDoorIndex >= 0
              ? group.salesDoors[salesDoorIndex]?.report?.pendingAssignment
              : group.report.pendingAssignment}
            )
          </span>
        </DropdownMenuTrigger>
      </Button>
      <DropdownMenuContent side={isMobile ? "bottom" : "left"} className="">
        <Form {...form}>
                ...
                {group.salesDoors
                  ?.filter(
                    (s, i) =>
                      ...
                      (salesDoorIndex >= 0 ? salesDoorIndex == i : true),
                  )
                  .map((salesDoor, index) => (
                    <div
                      className={cn(
                    ...
                    >
                      ...
                      {hands.map((h) => (
                        <div className="flex items-end" key={h.title}>
                          <FormInput
                            disabled={
                              salesDoor.report._unassigned[h.handle] == 0
                            }
                            control={form.control}
                            label={`${h.title} (${
                              salesDoor.report._unassigned[h.handle]
                            })`}
                            className=""
                            name={
              ...
              <Table className="hidden sm:table">
                ...
                <TableBody>
                  {group.salesDoors
                    ...
                    .map((salesDoor) => (
                      <TableRow className="" key={salesDoor.salesDoor.id}>
                        <TableCell>
                          <TableCol.Primary>
                            {salesDoor.doorTitle}
                          </TableCol.Primary>
                          <TableCol.Secondary>
                            {salesDoor.salesDoor.dimension}
                          </TableCol.Secondary>
                        </TableCell>
