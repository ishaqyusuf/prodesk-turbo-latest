  ...
  let doorGroups = fItems
    .map((item, index) => {
      ...
      if (!order.isDyke) {
        ...
        if (item.swing?.toLowerCase() == "lh") salesDoor["lhQty"] = item.qty;
        ...
        if (
          salesDoors.findIndex((s) => s.salesDoor.salesOrderId == item.id) == -1
        )
        ...
        salesDoors: salesDoors.map((sd) => {
          ...
          return {
            ...
            submissions: submissions.filter(
              (s, i) => submissions.findIndex((sq) => sq.id == s.id) == i,
            ),
          };
        }),
        report,
        formSteps: item?.formSteps,
        doorDetails: composeDoorDetails(item.formSteps as any, item as any),
      };
    })
    .sort(
      order.isDyke
  ...
  if (doorGroups.filter((dg) => dg.groupItemId).length > 0) {
    const ng = doorGroups.filter(
      (_, i) =>
        !_.groupItemId ||
        (_.groupItemId &&
          i == doorGroups.findIndex((d) => d.groupItemId == _.groupItemId)),
    );
        ...
        salesDoors: n.salesDoors.filter(
          (s, i) =>
            ...
            n.salesDoors.findIndex(
              (d) =>
                d.salesDoor.salesOrderItemId == s.salesDoor.salesOrderItemId,
            ),
        ),
      };
    });
  ...
  let splitted = [assignment.lhQty, assignment.rhQty].map((q, i) => {
    ...
    const r = {
      ...
      __report: {
        ...
        submitted: sum(
          assignment.submissions.map((s) => (isLeft ? s.lhQty : s.rhQty)),
        ),
        pending: 0,
        handle: isLeft ? "LH" : "RH",
        total: 0,
