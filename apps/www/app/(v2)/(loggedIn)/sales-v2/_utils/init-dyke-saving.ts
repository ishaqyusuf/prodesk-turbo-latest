...
function initializeMultiComponent(data: DykeForm) {
  ...
  data.itemArray.map((item, index) => {
    ...
    components.map((c) => {
      ...
      if (!type.moulding && !type.service && clone.item.housePackageTool) {
        ...
        clone.item.housePackageTool.priceData = c.mouldingPriceData as any;
        ...
        if (clone.item.meta.doorType == "Moulding") moldingId = c.toolId;
        ...
        if (clone.item.housePackageTool) {
          ...
          if (type.moulding) {
            // console.log(c.priceTags);
            clone.item.housePackageTool.meta.priceTags = c.priceTags;
            ...
            clone.item.housePackageTool.dykeDoorId = c.stepProduct?.doorId;
          }
        }
      }
