...
export default function DispatchModal(props: Props) {
  ...
  return (
    <Modal.Content>
      <Modal.Header onBack={() => {}} title="Dispatch" subtitle="lorem ipsum" />
      <Tabs value={tab}>
      ...
      {tab != "select" && (
        <Modal.Footer>
          {tab == "inspect" && <Button onClick={inspect}>Proceed</Button>}
          {tab == "recipient" && <Button onClick={inspect}>Proceed</Button>}
        </Modal.Footer>
      )}
    </Modal.Content>
  );
