    ...
    <Modal.Content>
      <Modal.Header
        title={
          <>
            <span>
              With Stat: {data.data?.filter((s) => s.stat?.length).length}
            </span>
            <span className="mx-2">
              Without Stat: {data.data?.filter((s) => !s.stat?.length).length}
            </span>
          </>
        }
        subtitle={
          ...
          {data.data
            ...
            ?.map((order) => (
              <TableRow key={order.id}>
                <TableCell>
                  <TableCell.Primary>{order.orderId}</TableCell.Primary>
                </TableCell>
                <TableCell>
                  <TableCell.Secondary>{order.stat.length}</TableCell.Secondary>
                </TableCell>
                <TableCell>
                  <Button
                    action={async () => {
