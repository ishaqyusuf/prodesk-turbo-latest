...
function OrderDispatch({ item, href }: SalesCellProps & { href? }) {
  return (
    <TableCell href={href} className="">
      <TableCell.Medium
        className={cn(item.isDyke ? "font-bold" : "", "whitespace-nowrap")}
      >
        {item.orderId}
      </TableCell.Medium>
  ...
  return (
    <TableCell href={link}>
      <TableCell.Medium className="uppercase">
        {item.customer?.businessName || item?.customer?.name || address?.name}
      </TableCell.Medium>
      <TableCell.Secondary>
...
function SalesRep({ item }: SalesCellProps) {
  return (
    <TableCell>
      <TableCell.Secondary
        className={cn(!item?.salesRep?.name && "text-red-500", "w-16 truncate")}
      >
  ...
  if (
    ...
    return (
      <TableCell>
        <TableCell.Money
          className={
            !item.amountDue ? "text-green-500 font-semibold" : "text-red-500"
          }
        >
          {item.grandTotal}
        </TableCell.Money>
    ...
    <TableCell>
      ...
      >
        {salesData.delivery.map((o) => (
          <MenuItem onClick={() => updateDeliveryMode(o.value)} key={o.value}>
            {o.text}
          </MenuItem>
        ))}
      </Menu>{" "}
...
function DeliveryAction({ item }: SalesCellProps) {
  ...
  return (
    <>
      <Button onClick={() => assignment.open(item.id)} variant={"outline"}>
        View
      </Button>
    </>
  );
