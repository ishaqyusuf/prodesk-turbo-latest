...
export function salesItemGroupOverviewDto(data: GetFullSalesDataDta) {
  ...
  function itemControl(uid, def: { produceable: boolean; shippable: boolean }) {
  ...
  const itemGroup = filteredItems.map((item, fItemIndex) => {
    ...
    function filterGroup(_item: Item, itemIndex) {
      ...
      return _item.qty && itemIndex >= startPointIndex && breakPointIndex > 0
        ? itemIndex < breakPointIndex
        : itemIndex > startPointIndex && !_item?.description?.includes("**");
    }
    ...
    groupedItems?.map((gItem) => {
      ...
      if (doors?.length) {
        doors?.map((_door) => {
          ...
          const pills = [
            createTextPill(inToFt(_door.dimension), _door.dimension, "blue"),
          ];
          let _totalQty;
          let totalQty: Qty = {};
          const isBifold = doorType == "Bifold";
          const controlUid = doorItemControlUid(_door.id, _door.dimension);
          ...
          if (isBifold || (!_door.lhQty && !_door.rhQty)) {
            ...
            pills.push(
              ...[
                createTextPill(`${_door.lhQty} LH`, _door.lhQty, "blue"),
                createTextPill(`${_door.rhQty} RH`, _door.rhQty, "orange"),
              ],
            );
          ...
          if (_door.swing)
            pills.push(createTextPill(_door.swing, _door.swing, "red"));
          ...
          itemAnalytics(
            {
              orderId: data.id,
              salesItemId: gItem.id,
              title: stepProduct.name || stepProduct?.product?.title,
              totalQty: {
                qty: gItem.qty,
                total: gItem.qty,
              },
        ...
        const produceable =
          gItem.dykeProduction || (!data.isDyke && gItem.swing != null);
          ...
          itemAnalytics(
            {
              ...
              total: gItem.total,
              pills: [
                createTextPill(gItem.swing, gItem.swing, "purple"),
                createTextPill(`qty x ${gItem.qty}`, gItem.qty, "purple"),
              ],
            },
            gItem.assignments,
