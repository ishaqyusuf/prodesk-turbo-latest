...
export function salesDispatchListDto(data: SalesDispatchListItem) {
  return {
    ...
    customer: {
      ...
      name: data.order.customer?.businessName || data.order.customer?.name,
      // name: data.order.
    },
    ...
    ?.map((grp) => {
      return grp?.items?.map((item, uid) => {
            ...
            return assignment.submissions
              .map((sub) => {
                ...
                assignment.deliveries
                  ...
                  .map((s) => {
                    pendingDelivery = qtyDiff(pendingDelivery, s.qty);
                  });
                return {
                  qty: pendingDelivery,
                  subId: sub.id,
        ...
        deliverableSubmissions?.map((s, i) => {
          if (i > 0) deliverableQty = qtyDiff(deliverableQty, s, true);
        });
        return {
