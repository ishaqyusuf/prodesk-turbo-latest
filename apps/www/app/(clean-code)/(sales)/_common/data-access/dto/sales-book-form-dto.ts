    ...
    return {
      ...
      formSteps: item.formSteps
        .map((item) => ({
          ...
          step: {
            ...item.step,
            meta: (item.step.meta || {}) as any as DykeStepMeta & StepMeta,
          },
        }))
        .filter(
          (f, fi) =>
...
export function transformShelfItem(item: SalesFormItems[number]) {
  ...
  const sorted = shelfItems.sort((a, b) => a.meta.itemIndex - b.meta.itemIndex);

  sorted.map((line) => {
    const siblings = sorted.filter((a) => a.meta.lineUid == line.meta.lineUid);
    if (line?.id == siblings?.[0]?.id) {
      shelfItemsData.lineUids.push(line.meta.lineUid);
      shelfItemsData.lines[line.meta.lineUid] = {
        categoryIds: line?.meta?.categoryUid?.split("-")?.map((a) => Number(a)),
        productUids: [],
        products: {},
      };
