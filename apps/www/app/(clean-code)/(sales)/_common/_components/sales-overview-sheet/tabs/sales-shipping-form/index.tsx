        ...
        <Portal nodeId={"tabHeader"}>
          <div className="flex items-center py-2 border-b gap-4">
            ...
            <Button
              ...
              onClick={() => {
                form.setValue("selectAllToken", generateRandomString());
              }}
              variant="secondary"
              className=""
            >
  ...
  return (
    <div className={cn("border-b px-4 py-2", total > 0 && "bg-green-50")}>
      ...
      <div className="flex gap-4">
        <div className="">
          <CheckCircle
            className={cn("size-4", total > 0 ? "text-green-500" : "opacity-0")}
          />
        </div>
        <div className="space-y-2  flex-1">
          <div className="flex-1 cursor-pointer">
            ...
            <div className="text-sm uppercase font-semibold">{item?.title}</div>
          </div>
          <div className="flex gap-2">
            {shipInfo?.inputs?.filter((a) => a.available)?.length ? null : (
              <>
          ...
          <div className="flex gap-2">
            {shipInfo?.inputs?.map((input, ii) => (
              <QtyInput uid={item.itemControlUid} input={input} key={ii} />
            ))}
          </div>
        </div>
            ...
            <Menu.Item
              ...
              SubMenu={
                <>
                  ...
                  <Menu.Item
                    Icon={ClipboardList}
                    disabled={!shipInfo?.pendingProductionQty}
                    shortCut={<>{shipInfo?.pendingProductionQty}</>}
                  >
                    Assigned Productions
                  </Menu.Item>
                  <Menu.Item
                    Icon={ClipboardX}
                    disabled={!shipInfo?.pendingAssignmentQty}
                    shortCut={<>{shipInfo?.pendingAssignmentQty}</>}
                  >
