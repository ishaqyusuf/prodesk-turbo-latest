    ...
    <div key={ass.id} className="py-2 text-sm space-y-4 border-b">
      <div className="flex items-center gap-4">
        ...
        >
          ...
          {workers?.data?.map((w) => (
            <Menu.Item onClick={() => assignmentChanged(w.value)} key={w.value}>
              {w.label}
            </Menu.Item>
          ))}
        </Menu>
        ...
        <Form {...form}>
          <div className="space-y-2 border-t">
            <div className="mb-2 space-y-2">
              <QtyInput submitable={ass.pendingSubmission?.lh} label="lh Qty" />
              <QtyInput submitable={ass.pendingSubmission?.rh} label="rh Qty" />
              <QtyInput submitable={ass.pendingSubmission?.qty} label="qty" />
            </div>
            <Collapsible>
              ...
              <CollapsibleContent className="py-2">
                <FormInput type="textarea" control={form.control} name="note" />
              </CollapsibleContent>
            </Collapsible>
          ...
          render={(props) => (
            <Input
              ...
              onChange={(e) => {
                ...
                if (val > submitable) {
                  e.preventDefault();
                  toast.error("Submit cannot exceed assigned");
                  return;
                }
