    ...
    <AssignmentContext.Provider value={ctx}>
      <div className="" key={assignment.id}>
        <div className="flex border-b font-mono items-center p-1 gap-4">
          ...
          <SubmissionStatusPill assignmentIndex={index} qtyKey="qty" />
              ...
              <ConfirmBtn
                onClick={async (e) => {
                  await deleteAssignmentUseCase(
                    assignment.id,
                    itemCtx.item?.analytics?.control?.produceable,
                  );
                }}
                trash
              />
...
function SubmissionStatusPill({
  ...
  return (
    <Badge variant="destructive" className={cn("uppercase font-mono py-0.5")}>
