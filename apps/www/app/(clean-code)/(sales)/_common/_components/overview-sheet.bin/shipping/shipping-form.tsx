      ...
      <div
        ...
        onClick={() => {
          ...
          if (val && !itemForm?.deliveryQty) {
            form.setValue(`selection.${item.uid}.deliveryQty`, deliverableQty);
          }
        }}
      >
        <div className="flex w-full justify-start text-start">
          ...
          {itemForm?.selected ? (
            <CheckCircle2Icon
              className={cn("size-4", itemForm?.selected && "text-green-500")}
            />
          ) : (
            <></>
          )}
        ...
        <div>
          ...
          <div className={cn(hasHandle && "grid grid-cols-2 gap-4")}>
            {deliverableQty.lh || deliverableQty.qty ? (
              <NumberPicker
                inline={!hasHandle}
                label={`Delivery Qty ${item.hasSwing ? "(LH)" : ""}`}
                size="sm"
                control={form.control}
            ...
            {deliverableQty.rh ? (
              <NumberPicker
                inline={!hasHandle}
                label={`Delivery Qty ${item.hasSwing && "(RH)"}`}
                size="sm"
                control={form.control}
