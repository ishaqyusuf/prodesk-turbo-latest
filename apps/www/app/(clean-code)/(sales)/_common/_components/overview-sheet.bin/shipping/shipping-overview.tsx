...
export function ShippingOverview({}) {
  ...
  async function updateProgress(progress) {
    try {
      const resp = await updateDispatchStatusUseCase(shipping.id, progress);
        ...
        >
          ...
          <div className="px-4 sm:p-8s">
            {ctx.items?.map(({ item, qty, deliveries }, index) => (
              <div className="border-b p-2 bg-white rounded" key={index}>
                <div className="flex">
                  <span className="text-sm font-semibold">{item.title} </span>
                  <div className="flex-1"></div>
                  <Badge value={item.swing} variant="secondary" />
                  <Badge value={item.size} variant="secondary" />
                </div>
                <div>
                  {qty.rh || qty.lh ? (
                    <>
                      <Badge variant="secondary" suffix="RH" value={qty.rh} />
                      <Badge variant="secondary" suffix="LH" value={qty.lh} />
                    </>
                  ) : (
                    <Badge variant="secondary" prefix="Qty" value={qty.qty} />
                  )}
                </div>
              </div>
            ))}
