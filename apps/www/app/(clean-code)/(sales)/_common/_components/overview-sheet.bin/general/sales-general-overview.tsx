    ...
    <>
      <dl>
        <DataLine
          ...
          value={
            <div className="inline-flex gap-2 items-center">
              ...
              <Button size="sm" variant="outline" className="h-6" asChild>
                <Link
        ...
        <DataLine
          label="Total Invoice"
          value={
            <TCell.Money className="font-mono" value={item.invoice.total} />
          }
        />
        {item.isQuote ? (
          <></>
          ...
          <>
            <DataLine
              label="Paid"
              value={
                <TCell.Money className="font-mono" value={item.invoice.paid} />
              }
            />
            <DataLine
              label="Pending"
          ...
          {[item?.addressData?.billing, item?.addressData?.shipping]
            ...
            .map((address, index) => (
              <div className="text-sm" key={index}>
                ...
                <DataLine.Icon icon="user" value={address.name} />
                <DataLine.Icon icon="phone" value={address.phone} />
                <DataLine.Icon icon="address" value={address.address} />
              </div>
            ))}
        </div>
        {item.isQuote ? (
          ...
          <div className="px-4 sm:px-8">
            ...
            <SalesItemStatus
              status={
                overview?.stat?.calculatedStats?.prodCompleted ||
                item.stats.prodCompleted
              }
              title="Production Completed"
            />
            <SalesItemStatus
              status={
                overview?.stat?.calculatedStats?.dispatchCompleted ||
