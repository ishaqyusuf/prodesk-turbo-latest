...
export function PaymentTab({}) {
  ...
  return (
    <div>
      <div className="">
        <div>{/* {ctx.overview?.shipping?.dispatchableItemList} */}</div>
      </div>
    ...
    <div>
      <Form {...ctx.form}>
        <div className="flex sm:justify-end right-0 m-4 sm:m-8 sm:mb-16  absolute bottom-0 mb-16">
          <Card
            className={cn("shadow-xl w-96", ctx.waitingForPayment && "hidden")}
          >
            <CardHeader className="bg-muted p-4">Payment</CardHeader>
            ...
            <CardFooter className="flex gap-4">
              ...
              <Button
                disabled={ctx.inProgress || ctx.paymentMethod == "link"}
                action={ctx._pay}
                size="sm"
              >
                Proceed
          ...
          <div
            className={cn(
              "hidden",
              ctx.waitingForPayment && "block border shadow-sm rounded p-2",
            )}
          >
