...
export function useStepContext(stepUid) {
  ...
  const [filteredComponents, setFilteredComponents] = useState<ZusComponent[]>(
    [],
  );
  ...
  useEffect(() => {
    const _filtered = stepComponents.filter((s) => {
      const filters = [];
      if (q) filters.push(s.title?.toLowerCase()?.includes(q?.toLowerCase()));
      switch (tab) {
        case "hidden":
  ...
  const salesMultiplier = useFormDataStore((s) => s.metaData?.salesMultiplier);
