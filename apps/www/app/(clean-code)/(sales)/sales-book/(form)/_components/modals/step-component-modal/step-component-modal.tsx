...
export default function StepComponentModal({ item, ctx }: Props) {
  ...
  return (
    <RenderForm {...form}>
      <Modal.Content>
        <Modal.Header title="Edit Product" subtitle={item.product?.title} />
        <div>
          <Tabs value={tab} onValueChange={setTab as any}>
            <TabsList className="">
              <TabsTrigger value="general">General</TabsTrigger>
              {ctx.isDoor && <TabsTrigger value="price">Price</TabsTrigger>}
              <TabsTrigger value="step">Component Step</TabsTrigger>
              {!item.id ? (
                <>
                  <TabsTrigger value="deleted">Copy</TabsTrigger>
              ...
              <div className="grid grid-cols-2 gap-2">
                {ctx.isDoor ? (
                  <div className="col-span-2">
                    <Tabs className="w-full " defaultValue={door.priceTab}>
                      <TabsList>
                ...
                {stepS.fields.map((f, fieldIndex) => (
                  ...
                  >
                    ...
                    <FormSelect
                      ...
                      size="sm"
                      onSelect={(e) => {
                        const empties = stepS.fields.filter(
                          (f, fi) => !f.id && fi != fieldIndex,
                        );
                        const emptyLength = empties.length <= 0;
                        if (emptyLength) stepS.append({});
                      }}
