...
async function saveComponent(ctx: CtxProps, formData: StepProduct, form?) {
  if (form) {
    ...
    let _show = {};
    let valid = false;
    Object.entries(d).map(([k, v]) => v && (_show[k] = true) && (valid = true));
    if (!valid) {
      toast.error(
...
function onUpload(
  form,
  assetId,
  path: "product.img" | "product.meta.svg" | "product.meta.url" = "product.img",
) {
...
function getDykeStepState(ctx: LegacyDykeFormStepType) {
  ...
  _formSteps.map((step, i) => {
    if (stateDeps?.[step.uid]) {
      stateBuilder = [stateBuilder, step.prodUid].filter(Boolean).join("-");
      states.push({
        step,
        steps: stateBuilder
          ?.split("-")
