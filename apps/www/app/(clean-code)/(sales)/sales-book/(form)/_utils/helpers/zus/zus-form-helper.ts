...
export function zhInitializeState(data: GetSalesBookForm, copy = false) {
  ...
  const resp: SalesFormZusData = {
    ...
    metaData: {
      ...
      sameAddress:
        data.billingAddress?.id == data.shippingAddress?.id || !data?.order?.id,
      billing: {
  ...
  data.itemArray.map((item) => {
    ...
    item.formStepArray.map((fs, i) => {
      ...
      const c = Object.entries(data.salesSetting.stepsByKey)
        .map(([k, s]) => s.components.find((s) => s.uid == componentUid))
        .filter(Boolean)[0];
      ...
      if (stp.title == "Item Type") {
        ...
        noHandle =
          resp.setting.composedRouter[stp.componentUid]?.config?.noHandle;
      }
      if (stp.title == "Door") {
        fallBackDoorStepProd = Object.values(data.salesSetting.stepsByKey)
          .map((s) => s.components)
          .flat()
    ...
    function addFormItem(formId, formData: GroupType["form"][""]) {
      formData.primaryGroupItem = formData.meta.salesItemId == item.item.id;
    ...
    if (item.item.shelfItemsData?.lineUids?.length)
      ...
      Object.entries(item.multiComponent.components).map(([id, data]) => {
        let sp = item.item?.housePackageTool?.stepProduct || data.stepProduct;

        if (!sp && fallBackDoorStepProd) {
          sp = fallBackDoorStepProd;
        }
        const stepProdUid =
          sp?.uid || item.item.housePackageTool?.door?.stepProducts?.[0]?.uid;
        ...
        if (doorCount) {
          setType("HPT");
          resp.kvFormItem[uid].groupItem.doorStepProductId = stepProductId;
          Object.entries(data._doorForm).map(([formId, doorForm]) => {
            ...
            addFormItem(formId, {
              ...
              pricing: {
                ...
                customPrice: customPrice(doorForm?.meta?.overridePrice),
          ...
          addFormItem(formId, {
            ...
            pricing: {
              customPrice: customPrice(data.priceTags?.moulding?.overridePrice),
              itemPrice: {
  ...
  const costCls = new CostingClass(new SettingsClass("", "", "", resp as any));
...
export function zhHarvestDoorSizes(data: SalesFormZusData, itemUid) {
  ...
  const validSizes = stepVar.variation
    .map((c) => {
      const rules = c.rules;
      const valid = rules.every(({ componentsUid, operator, stepUid }) => {
