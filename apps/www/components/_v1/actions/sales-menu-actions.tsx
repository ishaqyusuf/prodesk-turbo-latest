  ...
  return (
    <AuthGuard can={["editOrders"]} className="">
      <RowActionMoreMenu>
        ...
        <PrintOrderMenuAction link estimate={estimate} row={row} />
        <PrintOrderMenuAction mockup link estimate={estimate} row={row} />
        <PrintOrderMenuAction pdf estimate={estimate} row={row} />
...
export const PrintOrderMenuAction = typedMemo(
  (
    ...
    async function _print(mode: IOrderPrintMode) {
      ...
      if (props?.row?.deletedAt) (query as any).deletedAt = props.row.deletedAt;
      if (props.link) {
    ...
    <MenuItem
      ...
      SubMenu={
        <>
          ...
          <MenuItem Icon={Icons.flag} onClick={() => openAssignProd(row)}>
