...
function AutoComplete2({
  ...
  const filteredOptions = () => {
    ...
    let filteredOptions = items?.filter((option) => pattern.test(option.name));
  ...
  return (
    <div className="grid relative gap-2">
      ...
      <Combobox disabled={disabled} value={selected} onChange={valueChange}>
        <div className="relative mt-1">
          ...
          >
            ...
            >
              ...
              {(query?.length == 0 || (query?.length > 0 && allowCreate)) && (
                <Combobox.Option
