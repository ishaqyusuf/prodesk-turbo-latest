    ...
    <>
      <div className="col-span-2 grid gap-2">
        ...
        <DatePicker
          ...
          setValue={(e) => form.setValue(`costs.${fIndex}.startDate`, e)}
      ...
      {(community ? data?.project?.builder : data?.builder)?.meta?.tasks?.map(
        (t, _i) => (
          <div key={_i} className="col-span-4 gap-2 grid-cols-7 grid">
            ...
            <div className="col-span-2">
              <Input
                ...
                key="cost"
                className="h-8"
                {...form.register(`costs.${fIndex}.meta.costs.${t.uid}`)}
              />
            </div>
            <div className="col-span-2">
              <Input
                type="number"
                className="h-8"
                {...form.register(`costs.${fIndex}.meta.tax.${t.uid}`)}
              />
            </div>
          </div>
        ),
    ...
    <div className="sm:w-2/5 space-y-2 pr-2">
      ...
      <ScrollArea className="max-h-[350px] h-[350px] w-full">
        <div className="divide-y">
          ...
          {fields.map((f, i) => (
            <div className="flex items-center space-x-2 group mr-2" key={i}>
              <Button
              ...
              >
                <div className="flex justify-between flex-1 space-x-4">
                  <div className="text-start">{f.title || "New Cost"}</div>
                  <div>
                    <Badge className="" variant={"outline"}>
                      {form.getValues(`costStats.${f._id}`) || 0}
                    </Badge>
                  </div>
                </div>
              </Button>
              <ConfirmBtn
                ...
                onClick={async () => {
                  if (index == i && fields.length > 1) {
                    if (index > 0 && fields.length - 1 == index)
                      changeIndex(i - 1);
                  }
                ...
                size="icon"
                className={cn(
                  community && "group-hover:opacity-100 opacity-20",
                )}
                trash
              ></ConfirmBtn>
            </div>
