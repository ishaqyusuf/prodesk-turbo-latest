              ...
              >
                <div className="flex justify-between items-center flex-1 space-x-4">
                  <div className="flex flex-col items-start">
                    ...
                    <div>
                      <Money value={f?.meta?.grandTotal}></Money>
                    </div>
                  </div>
                  <div>
                    <Badge className="" variant={"outline"}>
                      {form.getValues(`stats.${f._id}`) || 0}
                    </Badge>
                  </div>
                </div>
              </Button>
              <ConfirmBtn
                onClick={async () => {
                  if (watchIndex == i && fields.length > 1) {
                    if (watchIndex > 0 && fields.length - 1 == watchIndex)
                ...
                variant="ghost"
                size="icon"
                className={cn("group-hover:opacity-100 opacity-20")}
                trash
              ></ConfirmBtn>
            </div>
          ))}
