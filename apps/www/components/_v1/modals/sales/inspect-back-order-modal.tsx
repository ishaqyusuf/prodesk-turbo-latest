...
export default function InspectBackOrderModal() {
  ...
  return (
    <BaseModal<TruckLoaderForm>
      ...
      Title={({ data: order }) => <div className="">Inspect Back Order</div>}
      Footer={({ data }) => (
        <>
      ...
      Subtitle={({ data }) => (
        <div>
          All items not loaded fully are backorders, click on other items you
          wish to add to backorder.
        </div>
      )}
      Content={({ data }) =>
        data && (
  ...
  return (
    <TableRow
      className={cn(
        ...
        backQty && "bg-orange-200 hover:bg-orange-200 cursor-not-allowed",
      )}
      key={item.id}
    >
      <TableCell
