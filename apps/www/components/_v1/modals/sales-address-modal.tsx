...
export function SalesCustomerModal({
  ...
  const submit = () => {
    loader.action(async () => {
      ...
      const { customerId: scid, search: ssea, ...siad } = shippingAddress || {};
      const _form = {
        ...formData,
  ...
  function getAddressLine(type: AddressType) {
    ...
    return {
      name: getValues(`customer.businessName`) || getValues(`${type}.name`),
      phoneNo: `${getValues(`${type}.phoneNo`)} ${phone2 ? `(${phone2})` : ""}`,
      // phoneNo2: ,
      address: getValues([
        `${type}.address1`,
        `${type}.city`,
        ...
        <div className="col-span-2 grid gap-2">
          <div className="col-span-3 ">
            <AutoComplete2
              ...
              itemValue="search"
              onSelect={(e) => {
                const { data: address } = (e as any) || ({} as any);
                console.log(address);
                if (typeof address === "object") {
                  const { customer, businessName, search, ..._address } =
                    address;
