...
export default function AutoComplete({
  ...
  const transformedOptions = transformItems(options || [], itemText, itemValue);
  useEffect(() => {
    resetOptions(options);
  }, [options]);
  function resetOptions(_opts) {
    const transformedOptions = transformItems(_opts || [], itemText, itemValue);
  ...
  function init(list?) {
    ...
    if (itemText != itemValue) {
      ...
      let v = ls.find(
        (item) =>
          String(item.value)?.toLowerCase() == String(text)?.toLowerCase(),
      );
  ...
  const {
    ...
    onInputValueChange({ inputValue }) {
      ...
      // console.log(value);
      if (allowCreate) onChange && onChange(value ? value.value : inputValue);
    },
    ...
    onIsOpenChange(changes) {
      if (changes.isOpen) {
        ...
        let value = items.find((item) => item.title == changes.inputValue);
          ...
          <>
            {items
              ...
              .map((item, index) => (
                <li
                  className={cn(
                    highlightedIndex === index && "bg-primary text-accent",
                ...
                >
                  <span
                    className={cn(uppercase && "uppercase", "line-clamp-1")}
                  >
                    {item?.title}
                  </span>
                </li>
            ...
            {rowVirtualizer.getVirtualItems().map((vi, index) => (
              <li
                className={cn(
                  ...
                  selectedItem === items[vi.index] && "font-bold",
              ...
              >
                <span className={cn(uppercase && "uppercase", "line-clamp-1")}>
