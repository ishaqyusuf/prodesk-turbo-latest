...
export function SendEmailTemplateSection() {
  ...
  function init() {
    getEmailTemplates(form.getValues("template.type")).then((data) => {
      ...
      setTemplates([{ title: "New", id: -1, subject: "", html: "" }, ...data]);
    });
  }
  useEffect(() => {
    init();
  ...
  useEffect(() => {
    ...
    setTab(templateId > 0 ? "default" : templateId == -1 ? "new" : "default");
    ...
    <div className="col-span-2">
      <Tabs className="" value={tab}>
        ...
        <TabsContent className="flex items-end gap-2" value="new">
          ...
          />
          <div className="flex space-x-2">
            <Button onClick={saveTemplate} size={"sm"} className="bg-green-600">
              Save
            </Button>
            <Button
              onClick={() => {
