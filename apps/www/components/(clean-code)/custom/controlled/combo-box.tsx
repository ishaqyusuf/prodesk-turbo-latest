            ...
            <Popover modal open={open} onOpenChange={onOpenChange}>
              <PopoverTrigger asChild>
                <FormControl>
                  ...
                  >
                    {selectedValues?.size == 0 || maxSelection == 1 ? (
                      <>
                        <span className="truncate">
                          {optLabel(
                            filterFields.find(
                              (opt) => optValue(opt) === field.value,
                            ),
                          ) || "Select field"}
                        </span>
                          ...
                          <div className="flex items-center">
                            ...
                            <div className="hidden min-w-0 gap-1 lg:flex">
                              {selectedValues.size > maxStack ? (
                                ...
                                filterFields
                                  ?.filter((option) =>
                                    selectedValues.has(optValue(option)),
                                  )
                                  .map((option) => (
                                    <Badge
                                      variant="secondary"
              ...
              <PopoverContent align="start" className={cn("min-w-40 p-0")}>
                <Command>
                  <CommandInput
                      ...
                      {filterFields.map((opt) =>
                        maxSelection > 1 ? (
                          <FacetedFilterItem
                            ...
                            value={optValue(opt)}
                            onSelect={onSelect}
                            selected={selectedValues.has(optValue(opt))}
                          >
                            {/* {opt?.icon && (
                                                            <opt.icon
