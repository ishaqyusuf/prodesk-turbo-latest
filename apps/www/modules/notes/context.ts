    ...
    getNotesAction(tagQuery).then((result) => {
      setNotes(
        result.filter((note) => {
          ...
          let status = note.tags.find((t) => t.tagName == "status")?.tagValue;
          if (status)
            validations.push(props.statusFilters.includes(status as any));
          let type = note.tags.find(
            (t) => t.tagName == ("type" as NoteTagNames),
          )?.tagValue;
          console.log([type, status, validations]);
          if (type) validations.push(props.typeFilters.includes(type as any));
