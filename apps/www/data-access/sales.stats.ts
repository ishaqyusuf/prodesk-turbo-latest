...
export async function updateSalesStat(id) {
  ...
  let totalQty = sum(
    data.isDyke
      ? [
          ...
          ...data.items.filter((i) => i.dykeProduction).map((s) => s.qty),
        ]
  ...
  await saveStat({
    ...
    score: sum(
      data.assignments
        ...
        .map((a) =>
          sum(a.submissions.filter((s) => !s.deletedAt).map((s) => s.qty)),
        ),
    ),
  ...
  await saveStat({
    ...
    salesId: data.id,
    score: sum(
      data.assignments.filter((s) => !s.deletedAt).map((s) => s.qtyAssigned),
    ),
  });
  await saveStat({
    ...
    score: data.deliveredAt
      ? totalQty
      : sum(data.itemDeliveries.filter((s) => !s.deletedAt).map((s) => s.qty)),
  });
}
